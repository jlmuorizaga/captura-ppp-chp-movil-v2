...
export class RelacionPromocionEspecialSucursalService {
  ...
  editaRelacionPromocionEspecialSucursal(registroRPES: RelacionPES) {
    return this.borraRelacionPromocionEspecialSucursal(registroRPES.idPromocion, registroRPES.idSucursal).pipe(
      switchMap(() => this.insertaRelacionPromocionEspecialSucursal(registroRPES))
    );
  }
}
